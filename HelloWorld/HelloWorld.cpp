...
std::vector<LPCWSTR> message_vec;               // vector where stored last 20 messages
...
int DisplayResourceNAMessageBox()
{
    int msgboxID = MessageBox(
        NULL,
        (LPCWSTR)L"user name = Hayk",
        (LPCWSTR)L"user surname = Poghosyan",
    ...
    case WM_PAINT:
        {
            ...
            for (int i = 1; i < message_vec.size() + 1; ++i) {
                if (i > 10) {
                    ...
                    TextOut(hdc, (Rect.right - Rect.left) / 2, (Rect.bottom - Rect.top - 100) - 25 * i, message_vec[i - 1], 3);
                }
            }
